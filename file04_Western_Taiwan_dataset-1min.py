...
def main():
    dates = ['20180317','20180319','20180320','20180322','20180324','20180326','20180328','20180330',\
             '20180403','20180404','20180407']
    date_nc, t_sec, lat, lon, H, H_round_to_hundred, u_ma, v_ma, w_ma,\
    p_H, static_P, dynamic_P, TAT, TD, THETA_aircraft, THETA_V, TV, TS, RELHUM,\
    MIXRATIOV_H2O, MIXRATIOM_H2O = HALO.adlr_data_csv('preprocess/others/loc_time_info_western_Taiwan.csv') 

    date_segment, t_seg = HALO.t_segment('preprocess/others/t_segment_western_Taiwan.csv') 
    ...
    fp = open('Analysis_dataset/Western_Taiwan_dataset-1min.csv','w')
    ...
    for i in range(len(t_sec_int)-1):
        if HALO.in_western_Taiwan_region(lat[int((t_ind[i]+t_ind[i+1])/2)],lon[int((t_ind[i]+t_ind[i+1])/2)]):
